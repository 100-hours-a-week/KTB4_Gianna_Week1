package org.example;

public class Constants {
    public static final String OPTION_SPITO = "1";
    public static final String OPTION_LOTTO = "2";
    public static final String OPTION_QUIT = "3";

    public static final int LOTTERY_COUNT = 6;

    public static final int SPITO_PRIZE = 1500;
    public static final int SPITO_MAX_NUMBER = 9;
    public static final int SPITO_WINNINGNUM_COUNT = 1;

    public static final int FRIEND_MAX_COUNT = 10;
    public static final int DAILY_STOCK_MAX_COUNT = 100;
}
